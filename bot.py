...
def tg_call(method: str, payload=None, timeout=30, retries=3, retry_delay=2):
...
def get_updates(offset=None, timeout=50):
    ...
    return tg_call("getUpdates", payload, timeout=timeout + 10, retries=2, retry_delay=3)
...
def handle_khl(chat_id: int):
    ...
    try:
        ...
        logger.error(f"Ошибка при формировании/отправке результатов КХЛ: {repr(e)}")
...
def handle_day(chat_id: int):
    ...
    try:
        ...
        logger.error(f"Ошибка при формировании/отправке игрового дня КХЛ: {repr(e)}")
...
def handle_testemoji(chat_id: int):
    try:
        ...
        logger.error(f"Ошибка при отправке custom emoji сообщения: {repr(e)}")
...
def run_bot():
    ...
    while True:
        try:
            ...
            logger.warning(f"Ошибка в polling, повтор через 10 секунд: {repr(e)}")
            time.sleep(10)
